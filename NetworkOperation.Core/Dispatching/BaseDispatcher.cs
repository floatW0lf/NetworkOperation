...
namespace NetworkOperation.Core.Dispatching
{
    ...
    public abstract class BaseDispatcher<TRequest, TResponse> where TRequest : IOperationMessage, new()
        where TResponse : IOperationMessage, new()
    {
        ...
        private ConcurrentDictionary<uint, CancellationTokenSource> _cancellationMap =
        ...
        public BaseDispatcher(BaseSerializer serializer, IHandlerFactory factory, OperationRuntimeModel model,
            ILoggerFactory logger, DescriptionRuntimeModel descriptionRuntimeModel)
        {
        ...
        public async Task DispatchAsync(Session session)
        {
            try
            {
                await foreach (var rawMessage in session.Bytes)
                {
                    ...
                    var context = new RequestContext<TRequest>(request, session, description,
                        _descriptionRuntimeModel.GetByOperation(description.OperationType));
                    try
                    {
                        if (GlobalRequestFilter != null)
                        {
                            ...
                            {
                                await session.SendMessageAsync(_serializer.Serialize(response, session).To(),
                                    description.ForResponse);
                                continue;
                            }
                        }
                        ...
                        var rawResponse = await ProcessHandler(request, context,
                            CreateCancellationToken(request, description));
                    ...
                    {
                        Logger.LogInformation("Operation canceled: {Request}", request);
                    }
                    catch (Exception e)
                    {
                        try
                        ...
                        finally
                        {
                            ...
                            await session.SendMessageAsync(_serializer.Serialize(failOp, session).To(),
            ...
            {
                Logger.LogError(e,"Dispatch session error:");
            }
        }
        ...
        protected abstract Task<DataWithStateCode> ProcessHandler(TRequest header, RequestContext<TRequest> context,
            CancellationToken token);

        protected async Task<DataWithStateCode> GenericHandle<T, TResult>(TRequest header,
            RequestContext<TRequest> context, CancellationToken token) where T : IOperation<TResult>
        {
            ...
            return new DataWithStateCode(
                context.OperationDescription.UseAsyncSerialize
                    ? await _serializer.SerializeAsync(result.Result, context.Session)
                    : _serializer.Serialize(result.Result, context.Session), result.Status);
        }
