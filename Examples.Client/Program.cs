...
namespace Client
{
    class Program
    {
        ...
        private static async Task WithDI()
        {
            ...
            evtsession.SessionOpened += session => Console.WriteLine($"Session Opened {session.NetworkAddress}");
            evtsession.SessionClosed += session => Console.WriteLine($"Session Closed {session.NetworkAddress}");
