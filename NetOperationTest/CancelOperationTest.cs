...
namespace NetOperationTest
{
    public class CancelOperationTest
    {
        ...
        public void ExecutorCancelTest()
        {
            ...
            mockSession.Verify(c => c.SendToAllAsync(It.IsAny<ReadOnlyMemory<byte>>(), DeliveryMode.Reliable | DeliveryMode.Ordered), Times.Exactly(2));
        }
        ...
        public async Task DispatcherCancelTest()
        {
            ...
            mockSession.Verify(s => s.SendMessageAsync(It.IsAny<ReadOnlyMemory<byte>>(), DeliveryMode.Reliable | DeliveryMode.Ordered), Times.Never);
            mockSessionWithCancel.Verify(s => s.SendMessageAsync(It.IsAny<ReadOnlyMemory<byte>>(), DeliveryMode.Reliable | DeliveryMode.Ordered), Times.Never);
